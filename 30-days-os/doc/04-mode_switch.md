...
* 高地址的8～11位（共4位）是type字段和12位的S字段共同指定本描述符的类型，即系统段和非系统段（S为0表示系统段，S为1表示非系统段），非系统段又分为数据段，代码段（type来执行）等等。这里凡事硬件运行需要用到的东西称为系统段，软件需要的东西称为非系统段。针对非系统段来说TYPE同样会标识这个段是否可读可写可访问等权限。
