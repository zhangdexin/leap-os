...
```
看起来这里应该就是在屏幕上显示load error的信息吧
最开始error标签处，SI赋值了msg的标签的地址
putloop开始调用，SI中的地址取到的值赋给AL，SI加1，判断AL是0的话结束输出跳转到fin，非0的话，输出AL中保存的字符，如此循环，就可以吧load error和换行符打印出来，msg最后是0用来控制循环结束
fin是一个死循环，只执行让CPU睡眠（HLT）指令
